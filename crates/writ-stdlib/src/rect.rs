...
use crate::vector::{extract_f32, extract_vec2, extract_vec3, vec2_value, vec3_value};
...
impl WritObject for WritRectangle {
    ...
    fn set_field(&mut self, name: &str, value: Value) -> Result<(), String> {
        match name {
            "position" => {
                self.position = crate::vector::value_to_vec2(&value)?;
                Ok(())
            }
            "size" => {
                self.size = crate::vector::value_to_vec2(&value)?;
                Ok(())
            }
            _ => Err(format!("Rectangle has no field '{name}'")),
        }
...
impl WritObject for WritBoundingBox {
    ...
    fn set_field(&mut self, name: &str, value: Value) -> Result<(), String> {
        match name {
            "min" => {
                self.min = crate::vector::value_to_vec3(&value)?;
                Ok(())
            }
            "max" => {
                self.max = crate::vector::value_to_vec3(&value)?;
                Ok(())
            }
            _ => Err(format!("BoundingBox has no field '{name}'")),
        }
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_fn_in_module(
        ...
        "rect",
        writ_vm::binding::fn2(|min: Value, max: Value| -> Result<Value, String> {
            let mn = crate::vector::value_to_vec2(&min)?;
            let mx = crate::vector::value_to_vec2(&max)?;
            Ok(rect_value(WritRectangle {
                position: mn,
                size: mx - mn,
            }))
    ...
    vm.register_type("BoundingBox", |args| {
        let min = crate::vector::value_to_vec3(args.first().ok_or("BoundingBox: missing min")?)?;
        let max = crate::vector::value_to_vec3(args.get(1).ok_or("BoundingBox: missing max")?)?;
