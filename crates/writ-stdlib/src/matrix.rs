...
use crate::vector::{extract_f32, vec2_value, vec3_value};
...
impl WritObject for WritMatrix3 {
    ...
    fn call_method(&mut self, name: &str, args: &[Value]) -> Result<Value, String> {
        match name {
            ...
            "transformPoint" => {
                let v = crate::vector::extract_vec2(args, 0)?;
...
impl WritObject for WritMatrix4 {
    ...
    fn call_method(&mut self, name: &str, args: &[Value]) -> Result<Value, String> {
        match name {
            ...
            "transformPoint" => {
                let v = crate::vector::extract_vec3(args, 0)?;
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_fn_in_module(
        "Matrix3_rotation",
        "matrix",
        writ_vm::binding::fn1(|angle: f64| -> Result<Value, String> {
    ...
    vm.register_fn_in_module(
        "Matrix3_scale",
        "matrix",
        writ_vm::binding::fn2(|sx: f64, sy: f64| -> Result<Value, String> {
    ...
    vm.register_fn_in_module(
        "Matrix3_translation",
        "matrix",
        writ_vm::binding::fn2(|tx: f64, ty: f64| -> Result<Value, String> {
    ...
    vm.register_fn_in_module(
        "Matrix4_rotation",
        "matrix",
        writ_vm::binding::fn2(|axis: Value, angle: f64| -> Result<Value, String> {
            let ax = crate::vector::value_to_vec3(&axis)?;
    ...
    vm.register_fn_in_module(
        "Matrix4_scale",
        "matrix",
        writ_vm::binding::fn3(|sx: f64, sy: f64, sz: f64| -> Result<Value, String> {
    ...
    vm.register_fn_in_module(
        "Matrix4_translation",
        "matrix",
        writ_vm::binding::fn3(|tx: f64, ty: f64, tz: f64| -> Result<Value, String> {
    ...
    vm.register_fn_in_module(
        ...
        writ_vm::binding::fn3(
            |eye: Value, target: Value, up: Value| -> Result<Value, String> {
                let e = crate::vector::value_to_vec3(&eye)?;
                let t = crate::vector::value_to_vec3(&target)?;
                let u = crate::vector::value_to_vec3(&up)?;
...
impl writ_vm::IntoNativeHandler for RawFn {
    fn arity() -> Option<u8> {
        None
    }
    fn into_handler(self) -> writ_vm::NativeFn {
        self.0
    }
}
